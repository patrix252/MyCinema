...
public class Classi {
    ...
    public static void inviaEmail(ServletContext context, HttpSession session){
        ...
        try {
            new Classi().createPDF(absoluteDiskPath, session);
        } catch (DocumentException | IOException ex) {
            Logger.getLogger(Classi.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
    
    public void createPDF(String dest, HttpSession session) throws DocumentException, IOException {
        ...
        for(int i=0; i<postiRidotti.size(); i++){
            document.add(new Paragraph("    Posto: " + postiRidotti.get(i).getRiga() + " " + postiRidotti.get(i).getColonna(), FontFactory.getFont(FontFactory.TIMES, 19)));
        }
        ...
        for(int i=0; i<postiInteri.size(); i++){
            document.add(new Paragraph("    Posto: " + postiInteri.get(i).getRiga() + " " + postiInteri.get(i).getColonna(), FontFactory.getFont(FontFactory.TIMES, 19)));
        }
